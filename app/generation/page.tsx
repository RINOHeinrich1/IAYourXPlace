    ...
    return (
        <div className="flex">
                ...
                <div className="flex items-center gap-22 mb-8">
                    ...
                    {!hasGeneratedImage && (
                        <div className="flex gap-11">
                            ...
                            >
                                Image
                                <span className={`absolute left-0 -bottom-1 w-full h-[2px] ${mode === 'image' ? 'bg-red-600 text-white' : 'bg-transparent text-gray-400 hover:text-white'} transition-colors`}></span>
                            </button>
                            ...
                            >
                                Vidéo
                                <span className={`absolute left-0 -bottom-1 w-full h-[2px] ${mode === 'video' ? 'bg-red-600 text-white' : 'bg-transparent text-gray-400 hover:text-white'} transition-colors`}></span>
                            </button>
                        </div>
                    )}
                </div>
                    ...
                    {imgSrc && (
                        ...
                        <div className={`rounded-3xl overflow-hidden relative w-1/3`}>
                            ...
                            <div
                                ...
                                style={{
                                    background:
                                        'linear-gradient(180.83deg, rgba(0, 0, 0, 0.087) -8.53%, rgba(0, 0, 0, 0.58) 99.29%)',
                                }}
                            >
                                <span className="text-white text-3xl font-bold font-montserrat font-medium">
                                    Mila nowak
                                </span>
                            </div>
                        </div>
                    ...
                    <div className={`relative flex flex-col ${hasGeneratedImage ? 'w-1/3' : 'w-2/3'}`}>
                        ...
                        <div className="absolute top-6 left-6 pointer-events-none">
                            <Image
                                ...
                                height={28}
                                style={{
                                    filter: "brightness(0) saturate(100%) invert(58%) sepia(0%) saturate(0%) hue-rotate(0deg) brightness(100%) contrast(88%)",
                                }}
                            />
                        </div>
                    </div>
          ...
          `}
                ...
                {/* Ligne de 5 images (plus grandes + serrées) - VISIBLE DANS LES DEUX MODES */}
                <div className="flex gap-2 mb-10 justify-between">
                    {bottomImages.map((img) => (
                        <div key={img.id} className="relative flex flex-col items-center cursor-pointer">
                            <div
                ...
                {/* Bloc "Nombre d'images" - MASQUÉ EN MODE VIDÉO */}
                {mode === 'image' && (
                    <>
                        ...
                        <div className="flex gap-4 mb-8">
                            {smallCards.map((card, index) => (
                                <div
                                    key={card.id}
                                    className={`flex items-center gap-2 w-[82px] h-[43px] rounded-xl p-2 justify-center ${index === 0 ? 'bg-red-600' : 'bg-[rgba(87,87,87,1)]'
                                        }`}
                                >
