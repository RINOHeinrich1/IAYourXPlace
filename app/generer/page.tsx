  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 ml-77 p-8 text-white bg-black min-h-screen relative">
        ...
        {/* Titre */}
        <h1 className="font-montserrat text-3xl font-bold mb-2 mt-22">
          Générateur d'Images et vidéos
        </h1>
        ...
        ${index === 0 ? 'text-white font-bold  after:absolute after:left-0 after:bottom-0 after:w-full after:h-[2px] after:bg-red-600'
                  : 'text-gray-500 hover:text-white'}
      `}
            >
              {post.title}
              {/* Ligne rouge au survol pour les autres */}
              {index !== 0 && (
                <span className="absolute left-0 bottom-0 rounded-xl w-0 h-[33px] bg-red-600 transition-all duration-300 group-hover:w-full"></span>
              )}
            </Link>
          ))}
        </div>
        ...
        <div className="p-10 bg-black min-h-screen text-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-9">
            {images.map((img) => (
              <div
                key={img.id}
                className="relative w-55 h-66 rounded-5xl overflow-hidden cursor-pointer"
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background:
                      'linear-gradient(199deg, rgba(255,255,255,0.25) 88%, rgba(255,255,255,0.1) 190%)',
                  }}
                >
          ...
          {selectedImage && (
            ...
            >
              <div className="flex flex-col -mt-22 items-center justify-center h-full relative">
                ...
                <button
                  onClick={() => {
                    router.push(`/generation?img=${encodeURIComponent(selectedImage.src)}&name=${encodeURIComponent(selectedImage.name)}`);
                  }}
